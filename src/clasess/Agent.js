...
class Agent {
  ...
  getTotalForYear(year) {
    return /^20(1[89]|2[0-5])$/.test(year)
      ? Math.floor(
          this.getTotalProfit(
            this.tripsData.filter((trip) => dayjs(trip.date)["$y"] === year)
          )
        )
  ...
  getTotalUserAverage() {
    ...
    let totalPerUserArray = Object.keys(usersObject).map((key) => ({
  ...
  getUsersCurrentlyTraveling() {
    return this.tripsData
      ...
      .reduce((acc, currentTrip) => {
        let userName = this.usersData.find(
          (user) => currentTrip.userID === user.id
        ).name;
        console.log(acc);
