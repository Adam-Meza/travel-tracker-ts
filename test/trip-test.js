...
describe("Trip Class", () => {
  ...
  it("should hold the data for the destination", () => {
    ...
    expect(trip.destination.location).to.equal("Wellington, New Zealand");
    ...
    expect(trip.destination.image).to.equal(
      "https://images.unsplash.com/photo-1442483221814-59f7d8b22739?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1950&q=80"
    );
    expect(trip.destination.alt).to.equal(
      "overview of city with buildings, water and trees"
    );
  });
